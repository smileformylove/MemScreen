...
class ChatPresenter(BasePresenter):
    ...
    def _is_vision_model(self, model_name: str) -> bool:
        name = model_name.lower()
        return (("vl" in name) or ("vision" in name) or name.startswith("qwen3-vl"))
    ...
    def _select_best_vision_model(self) -> str:
        ...
        for candidate in ["qwen3-vl:4b", "qwen3-vl:2b", "qwen2.5vl:7b", "qwen2.5vl:3b"]:
            if candidate in self.available_models:
                return candidate
        return self.current_model or "qwen3:4b"
    ...
    def _get_vision_model_candidates(self, prefer_fast: bool = False) -> List[str]:
        ...
        preferred = ["qwen3-vl:4b", "qwen3-vl:2b", "qwen2.5vl:7b", "qwen2.5vl:3b"]
        for model in preferred:
            if model not in vision_models and self._ensure_model_available(model, allow_pull=True):
                vision_models.append(model)

        if prefer_fast:
            ordered = ["qwen3-vl:2b", "qwen3-vl:4b", "qwen2.5vl:3b", "qwen2.5vl:7b"]
        else:
            ordered = ["qwen3-vl:4b", "qwen2.5vl:7b", "qwen3-vl:2b", "qwen2.5vl:3b"]
